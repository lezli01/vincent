...
runs on an **existing** branch instead of cutting one: it is fast-forwarded from
...
- The Output tab and the chat workspace share one renderer, and one verbosity
  ...
  becomes a terminal hyperlink only for sanitized http(s) links when you opt in
  with `tui.hyperlinks`, and is opened by vincent only when you pick it from a
  link picker that shows the whole destination first and opens nothing but
  `http` and `https`; a fenced block shows its
...
|---|---|
| Claude Code | Model and effort discovery, usage and cost reporting, restricted mode, mid-run questions, and the fullest account of its own run — the working directory and tool set it was given, its duration, turns, cache split and refused tool calls, the lines each edit changed with its hunks at `verbose`, and its subagents' work nested in the output pane rather than read as its own. Its remaining usage quota reaches vincent when you let it draw your status line |
...
a task's linked pull request from its Pull Request tab or the CLI — merge,
close, reopen, comment, and re-run failed Actions jobs (see the
[task workspace](guides/tui.md#task-detail) and
[`vincent github pr merge`](reference/cli.md#vincent-github-pr-merge)). Each
happens only when you ask for it — on the tab, each asks first — and
...
attempt while it is still running; `vincent chat transcript` does the same for
a chat's turn. `vincent task show <id> --step RUN` prints
