...
// live-output chunk types. The mapping itself is internal/agent's, shared with
// the chat runner so one wire format has one definition (task 071); events
// that carry nothing a client renders (results, errors — those surface as step
// outcomes) are skipped there.
func (r *Runner) publishAgentEvent(taskID, runID, offset int64, ev agent.Event) {
	for _, c := range agent.LiveChunks(ev) {
		r.publishOutput(taskID, runID, offset, c.Type, c.Payload)
	}
}
