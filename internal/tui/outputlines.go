...
// (task 124). It carries nothing: the prompt is already on screen as the
// chat's bubble or the step's prompt.
...
func outputLinesAt(records []apiclient.TranscriptRecord, seqs []int64, level outputLevel, width int, opts lineOpts) ([]string, []lineAnchor) {
	...
	for i := 0; i < len(records); i++ {
		...
		if rec.Type == recSubagentStarted || rec.Type == recSubagentProgress || rec.Type == recInputEcho {
			...
			// either: these were such lines until tasks 109 and 124 modeled
			// them.
			continue
		}
