...
`http` or `https` URL with a host and no `user@` part, at most 2048 bytes, made
of printable ASCII only. That refuses control characters and escape sequences,
spaces, and look-alike Unicode hosts. The link carries the destination exactly
as printed, and its id is generated by vincent rather than taken from the
agent. Every other destination renders as plain text. The numbered reference
list stays on screen either way, so a label that reads like one URL cannot hide
where a click goes. No remote image is ever fetched — there is no fetch in that path to
