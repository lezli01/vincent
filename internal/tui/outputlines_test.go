...
// TestInputEchoIsNeverALine is task 124 decision 24: cursor's echo of the
// prompt was an unrecognized line until it was modeled, and a modeled record
// that draws nothing must not split the run of unrecognized lines it sits in —
// one count of two, not two counts of one — nor add a line at verbose.
func TestInputEchoIsNeverALine(t *testing.T) {
	d := newTestDetail(t)
	d.width = 80
	before := apiclient.TranscriptRecord{Type: "agent.raw", Line: `{"type":"system","subtype":"init"}`}
