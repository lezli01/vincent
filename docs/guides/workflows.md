...
--agent claude-code --agent codex --agent cursor --yes --global`, which is the
published command with the interactive agent picker answered. `--agent`
narrows the selection to some of `claude`, `codex`, `cursor`; the TUI's `S`
offer answers it with the adapters vincent actually detected instead. Running
