...
- **Typing `@` in the TUI's chat composer picks a file from the chat's
  ...
  typed with the exact text that mentions that file — claude's
  `@"path with spaces"` quoting included, because the text comes from the
  daemon and nothing rebuilds it. Matching ranks a basename prefix over a
  path-segment prefix over a substring, and once you type a separator it
  matches the whole path, so `@internal/tui/chat` finds what you mean; both
  `/` and `\` separate, on every platform. A bare `@` opens nothing, which is
  what keeps `cc @someone` quiet, and neither does `user@host` — the picker
  reads the word under your cursor and it has to *start* with the `@`.
  Nothing is drawn when nothing matches, and the message is still sent
  exactly as you typed it: this is a typing aid, never a rewrite. The list
  and the skills list are never up together, and `↑`/`↓`, `tab`, `enter` and
  `esc` mean here what they mean there — `enter` with nothing highlighted
