...
func TestPaletteDisconnectedKeepsNavigation(t *testing.T) {
	...
	// Nine: the five takeovers, the chats board (task 067), the two
	// archived boards (task 092) and triggers (task 096.6). Navigation
	// survives a disconnect because reaching a screen is not an action on the
	// daemon — an archived board shows what it last fetched, the way every
	// other one does.
	if nav != 9 {
		t.Errorf("palette lists %d navigation entries while disconnected, want all 9", nav)
	}
}
