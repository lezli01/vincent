...
*Amended 2026-09-17 (task 115, issue #472): that sentence now describes a
...
| `state` | `idle` \| `running` \| `awaiting_input` \| `archived` \| `handed_off` \| `closed` (below; `closed` added 2026-09-17, task 115) |
...
| `linked_task_id` | *Added 2026-09-17 (task 115, issue #472):* the task this chat was opened on. Null for a free chat, fixed at open, and the **one authoritative foreign key** for the link, as `handoff_task_id` is for a handoff (task 074 decision 2): the lock (§6) and a task's `open_chat_id` (§13.2) are this column read backwards, never a stored copy. It selects the linked transition table below |
| `opening_context` | *Added 2026-09-17 (task 115):* the task context the daemon assembled when a linked chat was opened, prepended to the **first** turn's prompt and to no later one. A snapshot is exact because the task cannot move while the chat is open. Empty on a free chat |
...
| `archived` | terminal. The worktree is gone; nothing further can run in it. *Amended 2026-09-01 (task 074, issue #288): this was "the only terminal state"; there are now two.* *Amended 2026-09-17 (task 115, issue #472): there are now **three** — `closed` below, deliberately* |
...
| `closed` | *Added 2026-09-17 (task 115, issue #472):* terminal, and the only terminal state a **linked** chat can reach. The conversation is over; the worktree and branch it worked in were never its own, and closing touches neither. Not `archived`, because `archived` means "the worktree is gone", which closing must never make true; not `handed_off`, because nothing was transferred |
...
*Amended 2026-09-17 (task 115, issue #472).* `internal/chatstate` holds a
...
#### Linked to a task (added 2026-09-17, task 115, issue #472)
...
- **Its agent, model and effort resolve as a repair's do** (task 025 decision
  ...
  (§9.4) — a task that runs restricted does not get a full-auto chat.
...
> chats landed.
>
> *Amended 2026-09-17 (task 115, issue #472):* §6 changed when chats could be
...
| `chat` | blocked, awaiting_gate, done, aborted | *Added 2026-09-17 (task 115, issue #472).* Opens a §5.5 chat linked to the task, working in the task's existing worktree and branch (§13.2); no transition — the task stays where it is, **locked** against every other action but `cancel` until the chat is closed. Refused `409 task_has_no_worktree` on a task that never got one, and while a linked chat is already open |
...
*Amended 2026-09-17 (task 115, issue #472).* A chat **linked to a task** (§5.5)
...
- **A resumed codex run is always full-auto (2026-08-31, task 072 decision
  ...
  *Reopened 2026-09-17 (task 115):* a chat linked to a task takes the task's
  ...
  `restricted_unsupported`, and `POST /v1/tasks/{id}/chat` refuses the adapter
...
- **Chat worktrees (added 2026-08-30, task 063).** A chat (§5.5) gets a worktree
  ...
  *Amended 2026-09-17 (task 115, issue #472): a linked chat claims nothing.* A
...
*Amended 2026-09-17 (task 115, issue #472).* A turn of a chat **linked to a
...
GET    /v1/chats                        *Amended 2026-09-09 (task 092, issue #350).* Also takes
       ...
       &task_id=                        *Amended 2026-09-17 (task 115, issue #472):*
...
DELETE /v1/chats/{id}                   *Added 2026-09-09 (task 092, issue #350).* Permanent
                                        ...
                                        *Amended 2026-09-17 (task 115):* legal from `closed` as
...
POST   /v1/chats/{id}/archive           removes the worktree and deletes the branch when it
                                        ...
                                        *Amended 2026-09-17 (task 115):* on a live **linked**
...
POST   /v1/chats/{id}/handoff           *Added 2026-09-01 (task 074, issue #288).* Takes
                                        ...
                                        *Amended 2026-09-17 (task 115):* a live **linked** chat
                                        is `409 chat_linked_to_task` naming the task, replacing
                                        the "nothing to hand over" its empty `worktree_path`
                                        would otherwise produce
POST   /v1/chats/{id}/close             *Added 2026-09-17 (task 115, issue #472).* Ends a chat
...
GET    /v1/tasks/{id}                   full task incl. step runs summary and pending_input (§7.4).
                                        ...
                                        *Added 2026-09-14 (task 099, issue #430):* every task
                                        ...
                                        task 056 decision 4
                                        *Added 2026-09-17 (task 115, issue #472):* every task
...
POST   /v1/tasks/{id}/retry            { prompt_override?, run_override?, branch_override?, paused? }
                                        ...
                                        *Amended 2026-09-17 (task 115):* the cascade skips a
...
POST   /v1/tasks/{id}/chat             { title?, agent?, model?, effort? }
                                        ...
                                        2026-09-17, task 115, issue #472). Opens a §5.5 chat
                                        ...
                                        for an unregistered agent; `400 agent_cannot_resume`
...
*Added 2026-09-17 (task 115, issue #472).* **Three new stable error codes**, the
...
*Amended 2026-09-17 (task 115, issue #472).* One more durable kind,
...
*Amended 2026-09-17 (task 115, issue #472).* Two more, **thirty-four** in all:
...
CREATE TABLE chats (
    ...
    linked_task_id  INTEGER REFERENCES tasks(id) ON DELETE CASCADE,  -- task 115, migration 0032; NULL = a free chat
    opening_context TEXT,             -- task 115: prepended to a linked chat's first turn only
...
*Added 2026-09-17 (task 115, issue #472, migration 0032).*
...
8. **Chats board.** *Added 2026-08-31 (task 067, issue #269, closing 063.2).* A
   ...
   *Amended 2026-09-17 (task 115, issue #472).* A chat **linked to a task**
...
- **Container execution confines the filesystem, not the network or the
  ...
  still run on the host.* *Amended 2026-09-17 (task 115, issue #472): **free**
...
- **Per task:** aggregate duration/tokens/cost across attempts (rolled up from
  ...
  burned it. *Amended 2026-09-17 (task 115, issue #472):* a chat **linked to a
...
- **Retention:** transcripts of archived tasks pruned after
  ...
  **ended chat**'s turn transcripts (§5.5) — *amended 2026-09-01 (task 074): both
  ...
  retention window. *Amended 2026-09-17 (task 115): all three terminal states
