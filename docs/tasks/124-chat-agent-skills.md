...
124.1 was built, 20–27 when 124.2 was, 28 in its follow-up, and 29–37 when
124.5 was.
...
29. **2026-09-19 — `--message-file` on both `chat send` and `chat start`,
...
30. **2026-09-19 — Exactly one source of the message.** `chat send` is
...
31. **2026-09-19 — The bound is 4 MiB, `--fields-file`'s.** Chosen by the
    ...
    here.
32. **2026-09-19 — Input that is not valid UTF-8 is refused locally**, before
    ...
    is the only way to keep decision 29. A leading BOM is valid UTF-8 and
...
33. **2026-09-19 — Empty input is refused locally.** The daemon's `400
...
34. **2026-09-19 — The file is read before any request, in both commands.** A
    ...
    `--message` does.
35. **2026-09-19 — A local input error exits 1 as a plain `error` from
    `RunE`**, after task 045 decision 5. Exit 2 keeps its one meaning: no
    daemon answered.
36. **2026-09-19 — §12.1 gets a new row, not a note on an existing one.** The
    ...
    style.
37. **2026-09-19 — One bounded-read implementation.** `readInputFile`
...
- 124.5: `TestChatMessageFileReachesTheStoredPrompt` runs the real binary, a
  ...
  holds decision 30, `TestChatMessageFileRefusals` decisions 32–34 on both
  commands with no request and no chat, and `TestChatMessageFileBound`
  decision 31 at exactly the bound and one byte over. `TestReadFieldsFileBound`
