...
CREATE TABLE tasks (
  ...
  github_pr_json      TEXT,                   -- the pull request this task is linked to (§5.3, task 052,
...
- Task templates & recurring tasks; ~~issue-tracker ingestion (Jira → task)~~ —
  **the GitHub half promoted out of future work, 2026-08-26** (§5.3, §8.4,
  ...
  defined as "NULL = no linked issue" holding a bare `Issue`, so `github_pr_json`
