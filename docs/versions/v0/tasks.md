...
- *edit+retry:* rewrites that step's `prompt`/`run` inside `tasks.workflow_snapshot` (the snapshot stays the single execution truth, §5.3), plus a migration adding `step_runs.prompt_override TEXT` so the timeline can flag an edited attempt. **Spec addition** to §14.
