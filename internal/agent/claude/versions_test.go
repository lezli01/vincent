...
// the list ships empty because no claude release has been observed to break.
...
func TestDetectMissingBinaryHasNoVerdict(t *testing.T) {
	av, err := New(func() string { return "/nonexistent/claude-not-here" }).Detect(t.Context())
	if err != nil {
		t.Fatalf("Detect: %v", err)
	}
...
// TestCuratedRestrictedSupport pins claude as an adapter that can always
// restrict: --allowedTools is a flag every build has, on every platform.
func TestCuratedRestrictedSupport(t *testing.T) {
	...
	if !opts.RestrictedEverPossible() {
		t.Error("RestrictedEverPossible = false; claude restricts on every platform")
	}
}
