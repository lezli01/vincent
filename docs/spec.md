...
--agent <slug> [--agent <slug>…] --yes --global`. That is the published command
plus three flags, and the difference is required: `skills add` with no agent
selection opens an interactive multi-select, which cannot be driven from a TUI
takeover or a non-TTY CLI. The agent list is mapped through the table above,
one `--agent <slug>` pair per agent, and the two surfaces answer the picker
differently on purpose: the **CLI** defaults to all three adapters, because a
person who typed the command means "put it where my agents look" and `--agent`
is right there to narrow it, while the **TUI**
...
*Amended 2026-09-19 (issue #489).* The selection is spelled as one
`--agent <slug>` pair per agent, never as one comma-joined value. `skills add`
reads `--agent` as variadic (every following argument up to the next one that
starts with `-`) and accumulating across repeats; it does not split on commas,
and it matches each name exactly against its agent registry. The
`--agent claude-code,codex,cursor` that v0.9.0 shipped was therefore one
unknown agent, and every selection of more than one adapter failed with
`Invalid agents` and installed nothing. Of the two spellings the CLI accepts,
the repeated flag is the one its own README uses, and it binds each value to
its own flag, so a variadic run can never swallow a later argument.
