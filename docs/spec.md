...
`VINCENT_*` variable it gets. It runs under a fixed **1-minute** timeout that
...
GET    /v1/triggers                     *Added 2026-09-13 (task 096).* { enabled, dir, triggers[] }.
                                        ...
                                        is listed with its errors, never hidden; one that does
                                        not parse omits source_type, action_type, project_id
                                        and on_fire, which it has none of. `armed` is
...
- **Other warn lines.** A seed event whose `dedupe_key` does not render, a
  command output line that is not an event, and a cursor or ledger row that
  could not be written are each logged at warn.
