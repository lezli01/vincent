...
| 1 — Spine (M1) | 9 tasks | 1/9 | 🔨 in progress |
...
- [~] **T1.2 — SQLite store & migrations.** Pure-Go driver; WAL + busy_timeout; embedded migrations applying schema §14; typed store layer (CRUD + scheduler query) for projects, tasks, step_runs, events. *(Implemented + green locally on Windows; awaiting 3-OS CI matrix on the PR.)*
