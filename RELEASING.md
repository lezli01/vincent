...
6. **Watch the workflow.** `gh run watch`, or the Actions tab. Two stages:
   - `release` builds, signs the checksum file with keyless cosign, attests
     build provenance for every archive, and publishes the GitHub release.
...
8. **Read the published notes.** They are generated from the commits since the
...
- **Package-manager publishing.** No Homebrew tap, Scoop bucket or winget
  manifest yet; `go install` and the release archives are the supported paths.
