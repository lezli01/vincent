...
**This key is also the only gate on everything vincent writes to GitHub.**
...
pull request, and five more routes, driven by
[`vincent github pr merge`](cli.md#vincent-github-pr-merge), `close`, `reopen`,
`comment` and `rerun`, act on a task's linked pull request. There is
deliberately no second switch for any of them: the consent is a person asking,
not a line in this file nobody would turn on, so `enabled: false` turns them off
along with every read. An agent cannot reach them, since none of those routes is
an MCP tool.
