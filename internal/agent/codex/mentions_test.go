...
// TestFileMentionSyntax pins §9.3's mention syntax for codex (task 126):
// sigil `@`, recognized anywhere in the message, and `Expands: false` —
// through `codex exec` a mention arrives as prose, and the model reads the
// path itself in one tool call. Observed on codex-cli 0.154.0.
...
// TestFileMention pins codex's answer over the same input list claude's and
// cursor's tables run, so the three adapters are comparable row for row:
// codex quotes none of them, including the two claude quotes.
