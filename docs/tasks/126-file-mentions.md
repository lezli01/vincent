...
- [x] 126.13 (#556) The TUI guide, §15, `docs/features.md`'s one sentence
      ...
      `do_capture`. Then the capture itself: vhs 0.12.0 never starts ffmpeg
      on the maintainer's machine and every tape silently produces nothing,
      so `docs/assets/tui-chat-files.png` was taken with a vhs 0.11.0
      binary, and the guide's image reference landed in the same commit as
      the asset rather than ahead of it. The documentation
      ...
      `%d of %d` format never writes. ✓ 2026-09-23
