...
[CLI reference](../reference/cli.md#what-only-the-tui-does) lists. That
includes the human actions on a live task — approve, reject, retry, repair,
skip, pause, resume, answer, archive — so the gate you are about to approve by hand can also be approved from a script:
