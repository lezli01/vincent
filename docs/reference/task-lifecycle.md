...
| `archived` | Terminal. Worktree removed, record kept — until a human discards it with [`vincent task delete`](cli.md#vincent-task-delete), which is not an action on this page's list and is the only thing that removes the row. A deleted task can be brought back from a backup with [`vincent task import`](cli.md#vincent-task-import). The branch is kept unless it has no commits past its base, in which case it is deleted ([`delete_empty_branch_on_archive`](configuration.md#delete_empty_branch_on_archive)) | no |
...
| `fan_out_invalid` | A **derived** `fan_out` list is not a lane list: an item that is not a JSON object (the message names the line), a rendered lane id that is not a slug, two items rendering to the same id, or a `needs:` edge to a lane the step does not declare, or a cycle among them. Nothing is spawned. Every one of these is a load-time error for a declared `lanes:` list; a derived list has no ids until it is rendered, which is why it blocks here instead. Fix the producing step's output, or the lane template |
| `fan_out_limit` | A **derived** `fan_out` list is too long: past the step's `max_lanes:`, or past `fan_out.max_tasks` for the whole tree. Nothing is spawned. Raise `max_lanes:` on the step or `fan_out.max_tasks` in config, or narrow the list at its source. `fan_out.max_depth` is unaffected — it counts nesting, and a dynamic width does not nest |
...
| `mcp_unsupported` | The adapter cannot be given vincent's [MCP server](../guides/mcp.md) for this step. The step fails rather than running an agent that silently has no vincent tools. Set [`mcp.wire_steps: false`](configuration.md#mcp) if the step does not need them |
...
| `tree_cost_limit` | The task's whole fan-out tree — the root task and every lane below it at any depth, archived ones included — has spent past [`max_tree_cost_usd`](configuration.md#max_tree_cost_usd). The task that blocks is the one whose attempt crossed the line: usually a lane, sometimes the parent's own step before the fan-out or after the join. **Not a step failure**, exactly as `cost_limit`. Every task still working in the tree finishes one attempt and then blocks too, so expect to overshoot by up to one attempt per such task. Raise the cap and retry the parent, which re-admits every blocked lane at once, or retry one lane; without raising it each re-admitted lane buys one attempt and blocks again. When both caps are passed at once the reason is `cost_limit`. Counts only what agents reported |
...
| `input_unsupported` | The step declares `on_input: require` and its agent cannot take mid-run input. Refused at task creation, so only reachable when the agent changed underneath the task — or when the task runs [in a container](configuration.md#container): creation judges your host's CLI, the step the claude in the image |
