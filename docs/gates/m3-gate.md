...
| **S5** | "input-request alerts + answer form" | Covered by L8; additionally `t` in the form types a free-text answer instead of picking (it was `e` until task 093), and `esc` leaves without answering and the task stays parked. |
...
| **S9** | Projects view states | The spare repo gives the list a second row; `D` on a project with a running task is refused with a reason (it was `d` until task 093); `enter`/`e` edits and saves. |
...
| **S11** | Daemon view | The daemon entry in `:` shows version, uptime, pid, listen address, the three paths, `max_parallel_tasks: 3`, a database block whose footprint, per-table row counts and history span agree with what `vincent doctor` prints for the same daemon, both adapters with their resolved paths, and a log tail that matches what the daemon is actually writing. `R` refreshes all four. |
...
| **S15** | Resize the board and every task tab | Shrink and grow across 80×20 and 60×15 on the board, then in Steps & Attempts, Task Details, Output and Diff — the active task/tab, attempt selection, details scroll, output follow state, diff folds and any committed board filter survive; nothing panics or leaks a second task surface into the view. |
...
| **S17** | Accordion at 24 rows (T3.13) | On a 24-row terminal focus each panel in turn: the focused band expands, the collapsed band is title + one line, and the task table never shows fewer than five rows. *Rows, not tasks: since task 050 a task whose cells wrap takes up to three rows, so a band holding five rows can hold fewer than two tasks — that is the wrap working, not this leg failing. Judge the leg on rows.* |
...
| **S19** | The two creation forms' branch rows (task 125.9) | On the new-task form both branch rows open a list of the project's own branches: `/` narrows it, `t` types a name it does not offer. Choose the branch the project's checkout has out — the row and the Review stage both say the task will run in that checkout rather than in a worktree — then choose one nothing holds and create: the task runs on the branch that was already there and archiving it keeps the branch. Type a name into the free row instead and the task cuts a branch under it, unchanged. On the chats board, `n` → the `branch` row: the same list, adopt-only, and leaving it empty still has vincent cut `vincent/{id}-{slug}`. Starting a second chat on a branch a task already works in is refused on that row, with the reason. |
