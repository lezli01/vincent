...
var Excluded = []Route{
	...
	// A chat linked to a task (task 115) is still a chat: opening one starts
...
var routes = []Route{
	...
	{http.MethodPost, "/v1/tasks/{id}/repair", "task_repair", "Run one ad-hoc repair agent in a blocked task's existing worktree (§6, task 025). It does not re-run the step. Body: {prompt, agent?, model?, effort?}."},
