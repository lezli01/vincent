...
drives a real daemon against a real docker runtime; its header lists the nine
...
Scenarios 6–9 run `cmd/fakeagent`, cross-compiled for linux and bind-mounted
into the image as `claude`, so no real agent CLI is involved.
